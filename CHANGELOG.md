...
- 新增 `tests/test_webui_e2e.py` 7 项（integration）：真服务、真回环端口、真
...
- **抓到三条，前两条是真缺陷**：
  ...
  3. **假引擎不写 `answers`**：明细面板因此永远"无明细"，而点批次那一行是它唯一的
     入口。替身必须照 `question_stage` 写它写的那几行，连 `--no-record-text` 下该留
     NULL 的填空题一起写 —— "NULL 不许印成字面 `None`"那句断言从此才有对象可查。
- 顺带同一类：`set_weight_texts` 原来只改字典不发事件（等于第 1 条的另一种形态），
  现在写完广播整份快照。
- 数字：新增 13 项（E2E 7 + 对拍 4 + 会话回归 2）；离线套件 1647 → **1653 项**
  （CI 等价口径 `.venv-ci313`：1650 通过 + 3 跳过，跳过的仍是 opencv / openpyxl 那三条）、
  integration 25 → **32 项**，`scripts/e2e_gate.py` 数到 `ran=32 skipped=0`；
  TOTAL 90.7% → **91.1%**，`gui/app.py` 76.5% → **80.5%**（对拍把真窗口接进了跑批路径），
  `webui/` 99.1% 不变（1258 → 1270 句）；ruff / pyright 全 0；
  这 7 项 E2E 连跑三遍 **10.4~10.8 秒全绿**。
