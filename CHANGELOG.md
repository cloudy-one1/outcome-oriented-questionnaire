...
### 文档（README 瘦回标准仓库的形状，细节搬到 `docs/`）

- README 长到 663 行还在涨：参数一览表、权重 schema、工作原理全链路、术语表、落库结构、
  覆盖率缺口表全堆在首页，而这些是"查"的东西不是"读"的东西。现在首页只回答
  「这是什么 → 怎么跑起来 → 边界在哪 → 文档在哪」，其余搬到
  ``docs/cli.md``（CLI 全量：示例、参数、三态、断点续传、统计类开关、隐私）、
  ``docs/config.md``（权重配置与那条量表式矩阵的边界）、``docs/architecture.md``
  （工作原理 / 分层 / 题型边界 / 术语 / 落库）、``docs/coverage.md``（覆盖率口径与已知缺口）。
- 生成块跟着搬家：``scripts/readme_coverage.py`` → ``scripts/coverage_doc.py``，落点从
  ``README.md`` 改成 ``docs/coverage.md``。搬走块**不等于**放开首页手写数字的口子 ——
  "一位小数的百分比只许出现在生成块里"那条判据现在同时扫 README 与口径文档
  （``tests/test_doc_consistency.py``），并新增一条：README 里不许留哨兵。
- 首页不再手写测试条数（此前的"离线 1454 + 浏览器 35 = 1489"那组）。它们和覆盖率
  数字一样会腐烂，而唯一不会腐烂的说法是"跑一下看输出"；``docs/coverage.md`` 只保留
  与 skip 口径有关的那部分叙述，条数交回给运行结果。
- 题型数量的对拍从"只扫 README"扩到 ``docs/*.md``（``docs/design`` 与 ``docs/reviews``
  是当时的记录，冻结不扫）。顺手修了一个假阳性：``哪一类题型`` 里的"一"不是数量断言。
- 代码里指向已搬走章节的注释跟着改口（``src/config.py`` 的「方式一」、
  ``src/dialogs.py`` 的「仍然没有防线的地方」、测试里几处"README 缺口表"），
  历史叙事那类（"当年 README 写着 25% / 13%"）保留原话，只把时态钉死。
- 新增 ``tests/test_doc_links.py``：对用户讲话的那几页（README / CONTRIBUTING / SECURITY
  与 ``docs/`` 根下）里每个本地链接查三件事 —— 文件在磁盘上、锚点按 GitHub 的 slug 规则
  真存在、目标文件在版本库里。三条都是这次搬家自己踩到的：``docs/config.md`` 那个带
  ``matrix_scale`` 的标题算不出锚点；``.gitignore`` 的 ``docs/*`` 让新文档进了磁盘却没进
  仓库。三条各做过反向验证（改掉标题 / 链一个不存在的文件 / 链一个被忽略的草稿），
  分别让相应用例变红。
...
### 对拍（§4 第 4、5、11 行：配置工件、开机自动载入、孤儿批次）

- 四条新对拍都长在 `tests/test_host_parity.py`（现在 10 项）：
  - **同一张表按「导出配置」** → 两个宿主落到盘上的 JSON 除宿主标签之外逐键相等
    （`config`、`schema_version`、`meta.name`、`meta.survey_url`、题数；标签
    `GUI 导出` / `Web 界面导出` 明写成唯一允许的例外，与 `error_suffix` 同一手法）。
    导出是用户唯一能带走的工件，这里分叉等于两个界面互毁配置。
  - **交叉导入**：桌面版导出的文件给 webui 读、webui 导出的给桌面版读，
    两边最终的 `WEIGHT_CONFIG` 必须是同一份。
  - **开机静默载入默认配置** —— 没有对话框、没有人点，"打开工具就生效"那条路：
    同一份文件 → 分布相同、表格里的文本相同，而且**两句日志逐字相同**。
  - **孤儿批次收尾**：同一份"90 分钟前的 `running`"喂两个宿主，改判后的 `status` 与那句
    `[历史] 已把 1 个未正常收尾的批次改判为 failed` 都要一致。这条判错方向的代价是对称的：
    不改判 → 下次提示"从早已死掉的批次继续"，续传它可能直接重复提交；改判过头 →
    真该续的那批永远问不出来。
  - 外加一条措辞对拍：缺 `src/config_io` 时三句拒绝的话逐句相同，并且不许多做任何一步。
- 踩到的一处基础设施值得记：`WebService` 的 `save/load/validate_weight_config` 默认是
  `None`，由真入口 `webui/__main__.build_service` 注入。对拍里直接构造 `WebService` 而不注入，
  测到的其实是"缺依赖降级"那条分支 —— 症状是**导出静默不写文件**，红成
  `FileNotFoundError` 而不是红成"两边不一样"。第一版就是这样，加上 `CONFIG_IO` 注入才走到真路径；
  这条写进注释留在测试里。
- 四条都有牙，各改一处生产文案或退化一个方法验过：webui 导出标签改成 `GUI` → 红；
  桌面版缺依赖措辞改一个词 → 红；`auto_load_default_config` 改成立即 `return` → 红；
  `_reap_orphans` 那句"已把"改成"已将" → 红（红的时候两边原文都打出来，不用猜哪句变了）。
- 数字：离线 1690 → **1695 项**（CI 干净口径 1691 passed + 4 skipped）、integration 仍 33 项；
  TOTAL 91.13% → **91.39%**（这几条对拍真的在执行 controller 与 service 的导出/载入路径）；
  ruff / pyright 全 0。
- §11 条件①的进度：16 行里 **7 行**有跨宿主对拍。

### 对拍（§4 第 10 行：续传答"是"与答"否"，两个方向都钉）

- 上一条只钉了"答否"的一半；这次把**两个答案**都放进同一棵种好的历史库里对：
  同一条 `interrupted` 批次（计划 7 份、已成功 3 份、带着上次的权重），分别答"是"与答"否"，
  比两个宿主交给 `run_batch` 的 `resume_start_idx` / `attempts_cap` / `total_target` /
  `success_count` / `run_id` / `weight_config_snapshot` 与恢复出来的表格文本。
- 两个坏法方向相反，所以两个都要钉：把"不续传"写成"什么都不恢复" → 用户的权重像没存过；
  把"续传"写成从第 K 份（而不是 K+1）开始 → 平台上多一条**收不回来**的重复回收记录。
- **不只看"两边相等"**：每条分支各配一条绝对值断言（答"是"必须是 `4 / 4 / run_id == 那条批次`，
  答"否"必须是 `1 / 7 / run_id is None`）—— 只比相等的话，两个宿主一起错照样绿。
  这条纪律是 `Assertion reference frame` 那次教训的延续。
- 有牙，两处退化分别验过：`attempts_cap = planned - done` 改成 `planned` → 答"是"那格红；
  在"已忽略"那句之后加一行 `weight_config_snapshot = None`（"拒绝就全不恢复"那种 tempting
  写法）→ 答"否"那格红。两处都改回并确认工作树干净。
- 数字：离线 1695 → **1697 项**（CI 干净口径 1693 passed + 4 skipped）、integration 仍 33 项、
  合计 **1730**；TOTAL 91.39% → **91.40%**，生成块已 `--write`、`--check` 绿；ruff / pyright 全 0。
- §11 条件①的进度：16 行里 **8 行**有跨宿主对拍。

### 对拍与修复（§4 第 2、3 行：探测回流与扫码 —— 又抓到一条影响提交内容的缺陷）

- 探测对拍的做法是**同一台假 driver 喂两个宿主**（假件的 `counts` 按"主文档、iframe 0、
  iframe 1…"排布），比回流的题目表、第 4 列预填串、"探测完成，共 N 题（…）"那两句总结、
  以及 `driver.quit()` 次数；两种前置各跑一遍：空配置，和**先导入一份配置再探测**。
- 后半截抓到第二条真缺陷，而且影响的不是显示是**提交内容**：
  先导入配置再探测，webui 会**把刚导入的配置静默丢掉**。链路是三环扣一起的 ——
  `import_config` 只把权重写进"表上已有的行"（新会话里表是空的，等于什么都没写）→
  「探测题目」按等权重预填 → 点「开始运行」时把整张表重新解析并 `replace` 全局。
  桌面版不走这条路：它的 `populate` 默认值直接读全局 `WEIGHT_CONFIG`，所以导入的配置
  一直看得见、也留得住。修法是导入时**表上没有的题号也先记下**（表仍按探测结果渲染，
  不多出幽灵行），而那句"同步到表格 N 道"继续只数有行的，与桌面版逐字同口径。
- 有一条既有的单测 `test_import_syncs_only_questions_that_are_on_the_table` 钉的**正是这个
  错行为**（`assert 7 not in weight_texts`）—— 它当时是"照现状写"的，这次按新契约改名重写，
  并补一条"记下 ≠ 凭空多出一行"。**反向验过**：把 `import_config` 改回只写已有行，
  `[先导入再探测]` 那格立刻红、`[空配置]` 那格仍然绿。
- 其余钉住的东西：iframe 兜底比的是**在哪个帧里读的题**（两边都必须是命中的那一帧，
  收尾都切回主文档 —— 读错帧的样子是"探测成功但题是空的"）；找不到题目元素 / 验证超时 /
  driver 根本起不来这三种失败，两边的话术与"不落题、不谎报成功"一致；
  `_QUESTION_COUNT_JS` 与 `_TYPE_LABELS` 是**刻意复制**的两份（`webui/service.py` 顶部写了
  不 import gui 那一份的理由），复制就会漂，所以直接比这两个常量本身。
- 扫码那三格：模块整体没加载 → 两边都在**开选择框之前**就拒绝（桌面版真去开选择框就红）；
  解出 URL → 都写进字段并说同一句成功话；解不出来 → 同一句失败话，且**不许动用户原来的
  URL**。一处刻意不同没被抹平：webui 会把解出的内容过一遍 URL 校验（只放行 `http/https`），
  桌面版是直接 `url_var.set` —— 那属于 §5 的校验类修复，留在 webui 侧更强。
- 数字：离线 1697 → **1707 项**（CI 干净口径 1703 passed + 4 skipped）、合计 **1740 项**、
  integration 仍 33 项；TOTAL 91.40% → **92.47%**（两个宿主的探测主干路径第一次被真正走
  完，涨了 1.07pp）；ruff / pyright 全 0，生成块已 `--write`、`--check` 绿。
- §11 条件①的进度：16 行里 **10 行**有跨宿主对拍。

### 对拍（§4 第 12 行：日志的 tag 表、行号与不认识的 tag）

- 桌面版那张"每轮结果 → 日志级别"原本是 `on_round` 里的一个**内联字面量**，与
  `webui/service.py:_ROUND_LEVELS` 是两份抄本。先把它提出来成 `gui/app.py:_ROUND_LEVELS`
  才能逐键比 —— 不比就是自己等于自己。认不出的结果两边都退回 `INFO`：
  谁都不许把"未知"当成失败（当成失败会让人误停）。
- 同一条消息流喂两个宿主，比的是：行号连续 +1 且右对齐 4 位（桌面版 `f"{n:>4}"`、
  前端 `pad4`）、时间戳形状 `HH:MM:SS`、以及**不认识的 tag 谁都不许丢字** ——
  桌面版退成 INFO，webui 把原样交给 CSS、没有规则就落回默认色。
- 一条诚实记录：**4 位对齐此前没有任何测试钉着**。把 `f"{n:>4}"` 改成 `f"{n}"` 跑全量
  测试照样绿 —— 现在这条对拍是唯一证人。前端那半用浏览器 E2E 钉（`pad4` 改成 3 位
  立刻红），且必须读 `textContent`：Selenium 的 `.text` 会把前导空格缩掉，那时长相是
  "断言写错了"，实际是取错了属性。
- 三处退化分别验过：`_ROUND_LEVELS["unknown"]` 改成 WARN → 红；gutter 去掉 `:>4` → 红；
  `pad4` 改成 3 位 → 那条 E2E 红。
- 数字：离线 1707 → **1709 项**（CI 干净口径 1705 passed + 4 skipped）、合计 **1742 项**、
  integration 仍 33；TOTAL 92.47% 未动；ruff / pyright 全 0。
- §11 条件①的进度：16 行里 **11 行**有跨宿主对拍。

### 对拍（§4 第 9 行：进度条上那几个数的算法 —— B 段收口）

- "还没跑第 1 次就显示起点，否则显示 `resume_start_idx + current_attempt - 1`"这条规则
  在两个宿主里各有一份抄本（桌面版 `_sync_ui_mirrors_from_state`、webui `_sync_progress`）。
  按 6 组参数比镜像后的四个数与标签字符串，含 `total = 0` 不除零、续传还没开跑时要显示
  第 4 份而不是第 3 份。
- 有牙，且**验出"哪组参数才有牙"**：把 webui 的特例去掉，只有 `(resume=4, attempt=0)`
  那组红，`(resume=1, attempt=0)` 仍然绿（两者结果本来就相同）—— 这条规则只在续传时改变
  结果，用例里少了续传那组等于没测。
- §4 末尾新增「这 16 行的处置」清单：**12 行有对拍、1 行已是同一份实现（CSV 导出）、
  3 行刻意不同或纯显示层**（清理两步确认、历史列宽与上限、缺库黄条），每条写明理由。
  §11 的条件①从此可以按条核，不再是一句"逐条有对拍"。
- B 段一共抓到三条真缺陷，都不是"缺测试"而是"两份实现或两条路径真的走岔"：
  反向回填 webui 不建行、导入配置后探测会静默丢掉配置、日志 4 位行号此前没人钉。
- 数字：离线 1709 → **1715 项**（CI 干净口径 1711 passed + 4 skipped）、合计 **1748 项**；
  TOTAL 92.47% 未动；ruff / pyright 全 0。

### 变更（README 以 Web 控制台为主入口 —— §11 的条件③）

- 「界面」两节调换顺序并改衔：`### 界面：Web 控制台（主入口）` 在前，桌面版降为
  `### 界面：桌面版 GUI（参照实现，后续版本移除）`，并写明它退场的条件在设计稿 §11。
  Web 那节原来写的是"上面那七步一模一样"—— 读者得先读桌面版才看得懂主入口；
  现在反过来：Web 一节自己成立，桌面版那七步作为参照放在后面。
- 五处"GUI 独占"的措辞改掉：三入口写进功能特性与 `pyproject` 的 `description`；
  二维码解析注明两个宿主共用 `src/qr_utils.py`；权重表第 4 列编辑格式、历史的三种用法、
  续传确认框都改成按"界面"说（并标出清理在 Web 控制台里是两步确认）。
- **CHANGELOG 的历史条目一条都没重写** —— 那些行记的是当时的事实（"GUI 双入口"在 v3.0
  确实成立）。把旧条目改成"三入口"是伪造历史，要看的证据是这一条新增。
- 设计稿 §11 补上三条前提到 C 段结束时的状态：**① 按条有结论**（12 行对拍 + 4 行写明
  为什么不对拍）、**② 到位**、**③ 已改写**。于是 D 段（删）成为唯一剩下的动作，
  也是这一整程里唯一不可逆的一步。

### 转换（D 段第一步不是删，是把对拍的期望值降级成 webui 单侧契约）

- 为什么排在删前面：`tests/test_host_parity.py` 那 23 个函数的右边全是"另一个宿主给出的
  值"。`git rm gui/` 之后这些断言不会变红 —— 它们会**变成没人管**：期望值本身从没被写下来过，
  它只以"和那边一样"的形式存在。所以先把值就地钉成字面量，再动删。
- 新增 `tests/test_webui_host_contract.py`（20 个用例，断言右边不出现 `gui`）。23 个函数
  逐条查过去向：**14 个**改由这个文件用字面量钉住（表单 → `RunState` 的 8 个字段与交给
  `run_batch` 的 10 个关键字、`error_suffix` 那句宿主标签、权重表 → 快照、进度 6 组数、
  逐轮那一行的 `(✓n ✕m)`、结果级别表、导出文档的形状、导出件能被同一个宿主读回、导入的
  "整体替换而非合并"、开机自动载入、孤儿批次那句改判、扫码的三段与两种降级）；剩下
  **9 个**在 `tests/test_webui_service.py` 里早就是 webui 单侧断言（探测 5 例、续传的三个
  方向、`config_io` 缺席时的拒绝措辞、日志行号），这个文件不重复一遍。
- 两处是顺手补出来的**新契约**，因为量了一下发现删 Tk 会真的留下洞：
  `_QUESTION_COUNT_JS` 原来只和桌面版那份比"一模一样"，而这条判据只存在于对拍文件里 ——
  实测把 `div.question-rating` 或 `div.field div.label` 从 webui 那份里删掉，
  `test_host_parity.py` 红，`test_webui_service.py` 132 项与 E2E 8 项**全绿**
  （离线替身返回的是写死的题数，选择器少了哪家它不知道）。对拍文件一走，这条就没人看了，
  所以现在钉的是覆盖面本身（radio/checkbox/text/textarea/select、量表四写法、
  矩阵与排序共用的 `div.field div.label`）。
  另一处是 `stop_check` 的**内容**：对拍只检查"关键字里有这个名字"，把它换成恒 False 的
  占位符 30 个用例全绿 —— webui 侧另有 2 项和一条 E2E 能抓，所以这不算洞，
  补它是为了"宿主对引擎的全部责任"能在一处一口气读全（那是长跑唯一的刹车踏板）。
- 负向验证：18 处改坏（`aborted` 的级别、进度起点、导入改回合并、自动载入的守卫去掉、
  导出题数写死、题型中文名、宿主标签……），**17 处让这个文件变红**。没变红的只有一处：
  把 `RunState.snapshot_weight_config` 里的 `int(k)` 改成 `k` —— 全局 `WEIGHT_CONFIG` 的键
  在进快照前就已经是 int，那是个**等价变异**，不是洞。中途有三处先活了下来（`stop_check`
  的内容、自动载入那两条、"导入前先塞一份这份文件里没有的旧状态"），补了用例才有牙。
  做这件事的脚本是临时的，跑完即删。
- 顺带修掉一处类型门禁里的真错：`WebService.__init__` 把 `save_weight_config` 声明成
  `Callable[..., None]`，而 `src.config_io.save_weight_config` 返回落盘路径（`str`）。
  契约测试是第一个把**真函数**传进去的测试 —— 以前所有用例都传 lambda，那个签名一直是猜的。
- 数字：离线 1715 → **1735 项**（CI 干净口径 1731 passed + 4 skipped）、合计 **1768 项**；
  覆盖率口径由 `scripts/readme_coverage.py --write/--check` 重跑确认一致（92.5%，抖动内）。

### 修复（同一张权重表被推两遍：焦点掉了、敲到一半的字被换回去、门禁自己在 CI 上闪）

- 收 D-1 的账时顺手撞上的：整条浏览器 leg（`pytest tests/ -m integration`）**改之前五次里
  红两次**，而单独跑 `tests/test_webui_e2e.py` 永远绿。红点固定在探测那条，抛的是
  `StaleElementReferenceException` —— `wait >= 13 行` 是在**第一次**渲染上过的，紧跟着那条
  `state` 快照又把 13 行原样推了一遍，`renderTable` 无条件 `textContent = ""` 重建，
  手里的行就 stale 了。
- 人在界面上看到的不是异常，是同一件事的另一种形态：**探测完想接着改第 1 题的权重，
  点进输入框的那一瞬整张表被换成新节点，焦点掉、敲到一半的字被服务端的旧值盖回去、
  逐行入场动画重播一遍**。这类只有真浏览器 + 真 SSE 才现形，离线 1735 项对它一无所知。
- 改法是把"内容一样"这件事说出来：`renderTable` 先按 `q/type/n/label/text` 拼一个签名，
  与上次渲染相同就直接返回。表真的变了（改过权重、探测到另一套题、导入配置）照旧重建。
- 新增 `test_an_unrelated_state_event_leaves_the_table_alone`：给第 1 题那个 input 挂一个
  `data-marker`，去点一个与表无关的复选框（`no-record-text`，会经服务端推一整份 state），
  再回来读同一个选择器 —— 标记还在才说明节点没被换掉。**反向验过**：把那行守卫删掉，
  这条 5.95 秒就红。修完整条 leg 连跑三遍 34/34 全绿。
- 浏览器 E2E 33 → **34 项**（webui 那 8 条变 9 条），合计 **1769 项**。

## [3.4.0] - 2026-09-25

v3 第三程：**给同一台引擎再添一个宿主，并且先证明它和桌面版给出的是同一份东西**。
三件事合起来才是这一程：`gui/motion.py` + `gui/ticker.py`（Tk 侧的动效层，纯时序原语
+ 可停摆心跳）、`webui/`（stdlib HTTP + SSE 的本地控制台，第三个入口 `wjx-web`）、
以及钉住两者等价的那批测试。**号从"目标 3.3.1"改成 3.4.0**：未发布块里躺着的不是
修补，而是新宿主 + 新入口 + 两个新模块，按项目开头那句"遵循语义化版本"该走次版本
—— 本仓库历史上十二次发版也全是 `x.y.0`，没用过补丁位。

这一程守的仍然是同一句取舍，只是换了对象：上一程管的是"什么时候允许说话"，这一程管的是
**"什么时候允许换实现"**。三条判据各自落成一道闸 —— 换宿主前必须先跑通一次真实长跑
（设计稿 §10 的止损点）、合函数前必须先逐题型对拍（抓到三条才合并）、并进套件前必须先
在真浏览器里红过一次（这一轮抓到的四条全是"语法合法、Python 全绿、而界面上不成立"）。
反过来，明确不做的也写在纸上：不借 react-bits 的 WebGL 背景与光标物理、不做远程访问、
不删 `gui/`（推到赛后，且 §11 那三条前提目前只到一小块）。

数字：离线套件 1107 → **1653 项**、浏览器 E2E 25 → **33 项**（合计 1686），
`src/` + `gui/` + `webui/` 的 CI 口径总覆盖率 87.9% → **91.1%**，
ruff / pyright 全 0。
...
- 数字：新增 17 项；全套 1655 → **1672 项**（"离线"这个标签当时用错了 —— 1672 里含
  25 项 integration，离线可跑的那部分是 1647）；ruff / pyright 全 0；
  浏览器 E2E 两相 **46 项断言全绿**（第一相 39 项是原有的长跑 + 历史栏，
  第二相 7 项是这次的真确认往返）。

### 界面（第八个对标源：E2E 进套件 + 两宿主对拍 —— 步骤 8）

- 新增 `tests/test_webui_e2e.py` 8 项（integration）：真服务、真回环端口、真
  headless Edge、真 `EventSource` —— 替身只有 driver 工厂与探测/答题两处。设计稿 §9
  那句"前端 gate 有两条，`node --check` 只是第一条"到这里才补齐第二条：需要它的理由
  和 v3.0 那 4 条一模一样 —— 语法合法、Python 全绿、界面上不成立。
- 新增 `tests/test_host_parity.py` 4 项（离线，进阻塞门禁）：同一套表单值分别打到
  Tk 的 `_on_start` 与 webui 的 `start_run`，比的是**交给 `run_batch` 的那份东西**：
  位置参数、关键字集合（宿主标签是唯一允许的例外，明写成例外）、`RunState` 八个字段、
  以及权重表 → `weight_config_snapshot` 整条。宿主对跑批的全部责任就是填好这份东西，
  而它出错的样子是"两个界面跑出来的分布不一样"，没有任何一处会红。
- **抓到四条，前两条是真缺陷**：
  1. `emit("state", snapshot())` **漏号**：实参先求值，广播出去的载荷带着*取号之前*的
     rev，于是它和一份"在处理线程里早读好、晚一步才送到浏览器"的 HTTP 响应同号；
     前端只认号更大的，新事件被旧响应挤掉。症状是点「探测题目」之后权重表整块不出现，
     日志却写着探测成功 —— 而离线测试两边读的都是同一个 `snapshot()`，永远看不出分叉。
     修在 `webui/session.py`：新增 `emit_state()`，号与内容在同一次加锁里定下来，
     原来四处 `emit("state", snapshot())` 全部改走它。回归测试钉的是"一份在途响应
     与紧随其后的事件不许同号"（旧写法实测 `0 / 0 / 1`，新写法 `1 / 2 / 2`）。
  2. **历史栏在批次结束后不作废**：`runsLoaded` 一旦为真就再也不重拉，停在历史栏上
     刷新批次的人看到的永远是上一批 —— 桌面版是在 `on_run_finished` 里顺手刷的，
     这一条属于 parity 漏项。修在 `app.js`：`running` 由真变假的那一次渲染作废列表，
     并且只有当前正停在历史视图时才重拉（换视图再拉，不白传）。
  3. **替身不够真**，两条都是：假引擎不写 `answers`，明细面板因此永远"无明细"，
     而点批次那一行是它唯一的入口 —— 必须照 `question_stage` 写它写的那几行，
     连 `--no-record-text` 下该留 NULL 的填空题一起写，"NULL 不许印成字面 `None`"
     那句断言从此才有对象可查；假引擎也不轮询 `stop_check`，于是浏览器里那颗
     「停止」按钮测到的只是"按钮变灰了"。补上之后新增的那一条把 worker 按在
     第 1 轮与第 2 轮之间、人在页面上按停止再放行，钉住轮边界语义（`src/cli.py:724`
     那条一样的判据）、剩下两份不交出去（`1 / 3`）、收尾落库必须是 `interrupted`
     —— 标成 `finished` 的那批永远续传不了。反向验过：把页面上那一下点击删掉，
     这条立刻红（成功 3）。
- 顺带同一类：`set_weight_texts` 原来只改字典不发事件（等于第 1 条的另一种形态），
  现在写完广播整份快照。
- 数字：新增 14 项（E2E 8 + 对拍 4 + 会话回归 2）；离线套件 1647 → **1653 项**
  （CI 等价口径 `.venv-ci313`：1650 通过 + 3 跳过，跳过的仍是 opencv / openpyxl 那三条）、
  integration 25 → **33 项**，`scripts/e2e_gate.py` 数到 `ran=33 skipped=0`；
  TOTAL 90.7% → **91.1%**，`gui/app.py` 76.5% → **80.5%**（对拍把真窗口接进了跑批路径），
  `webui/` 99.1% 不变（1258 → 1270 句）；ruff / pyright 全 0。
  这 8 项 E2E 在 `.venv-ci313` 连跑四遍 **10.4~10.5 秒全绿**，加上原有 25 项的整套
  integration 连跑三遍全绿。
