...
# 离线套件（无浏览器环境 / CI，1085 项；只装 requirements*.txt 的口径下会有若干 skip）
...
当前测试全部通过：**1108 项**（离线 1085 + E2E 23）。
...
> |---|---|---|
> | 离线套件 | 1085 passed | 1082 passed + 3 skipped（二维码解析 2 项、`.xlsx` 真读 1 项；全新检出时 `coverage.json` 还不存在 —— 文档口径比对那一项也 skip，它是同一次运行**末尾**才产出的） |
