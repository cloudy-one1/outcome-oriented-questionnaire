...
- **有跨宿主对拍：12 行** —— 1 表单五字段、2 探测、3 扫码、4 配置导入导出、5 自动载入、
  ...
  **第 3 行（扫码）后来补了页面入口**：`POST /api/qr` 与 `import_qr` 一直都有测试，
  而 `webui/static/` 里连 `qr` 这个词都没出现过 —— 对拍打的是 service/HTTP 两层，
  浏览器用例只会点页面上**存在**的按钮，所以"控件缺失"这个形状谁都没看见，
  是用户对着界面问"为什么没有扫码的选项"才暴露的。现在按钮、`FileReader` 上传链
  与一条真选图的文件用例都在（`test_a_qr_image_picked_in_the_page_fills_the_url_field`）。
